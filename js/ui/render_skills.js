...
function renderSkills() {
  ...
  [['ATK', P.atk], ['DEF', P.def], ['MIND', P.mind],
   ['HP Max', P.maxHp], ['MP Max', P.maxMp], ['Crit %', Math.round(P.crit * 100) + '%']]
  .forEach(function (s) {
    html.push(
      '<div class="sbox">',
  ...
  SKILL_ORDER.forEach(function (sk) {
    ...
    var unlocked = meta.abilIds.filter(function (aid) {
      var ab = ABIL[aid]; return ab && ab.lv <= lv;
    });
    ...
    html.push(
      '<div style="margin-bottom:16px">',
        ...
        '<div style="margin-left:85px;margin-top:-4px;margin-bottom:6px;font-size:9px;color:var(--mut)">',
          nxtAb
            ? 'Next: ' + nxtAb.name + ' at Lv.' + nxtAb.lv + ' · ' + xpLeft + ' XP'
...
function renderSkillsSide() {
  return [
    ...
    '<div style="font-size:11px;color:var(--mut);line-height:1.9">',
      'Skills level 1–99 through combat use.<br>No point allocation.<br><br>',
      ...
      '<span style="color:#44ee66">',
        ...
        '📜 Lore → MIND, special',
      '</span><br><br>',
      'Crit chance: <span style="color:var(--gold)">' + Math.round(P.crit * 100) + '%</span>',
    '</div>'
  ].join('');
}